...
(68, 'vsvsd', 'svsdv', '234.00', 'sneaker-sale-vector-minimal-color-260nw-548194222.webp');
...
--  CREATE VIEW `special_product` AS 
-- (SELECT * FROM product, product_categories WHERE product_categories.productID = product.productID AND catID = 5);
...
-- $$
-- DELIMITER ;
-- DROP TRIGGER IF EXISTS `trigger_update_brand`;
-- DELIMITER $$
-- CREATE TRIGGER `trigger_update_brand` BEFORE UPDATE ON `brand` FOR EACH ROW SET NEW.title = UPPER(NEW.title)
-- $$
-- DELIMITER ;
